...
function expiryTone(v: string | null): [any, string] {
  ...
  const days = Math.round((new Date(v + "T00:00:00Z").getTime() - Date.now()) / 86_400_000);
...
export function VaultTable({ rows }: { rows: any[] }) {
  const cols: Column<any>[] = [
    ...
    { key: "state", header: "Status", width: "140px", align: "right",
      cell: (r) => { const [tone, label] = expiryTone(r.expires_at); return <Badge tone={tone}>{label}</Badge>; } },
  ];
