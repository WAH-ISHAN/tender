...
export function NavLink({ href, children }: { href: any; children: ReactNode }) {
  ...
  return (
    <Link href={href} className={`rounded-[8px] px-3 py-1.5 text-[13px] font-medium transition-colors ${active ? "bg-brand-50 text-brand-700" : "text-ink-600 hover:bg-ink-100 hover:text-ink-900"}`}>
      {children}
    </Link>
  );
}
